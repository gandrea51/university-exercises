...
int main() {
    ...
    // Inizializzazione
    init(grid);     // Impostazione della temperatura TC e gradiente verticale a destra
    ...
    for (iter = 1; iter <= MAXITER; iter++) {
        ...
        for (int i = HY; i < HY + GLY; i++) {
            for (int j = HX; j < HX + GLX; j++) {
                ...
                // Vincolo del bordo: controllo se la cella si trova nell'estremo destro dove non si effettua l'aggiornamento
